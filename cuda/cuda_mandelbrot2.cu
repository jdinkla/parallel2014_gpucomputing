...
__global__
void mandelbrot2_kernel(Extent ext, int* dest, const int max_iter, Rectangle r)
{
	...
	if (i >= 0)
	{
		...
		dest[i] = mandelbrot2(make_float2(x0, y0), max_iter);
	}
}
	...
	Rectangle r)
{
	...
	mandelbrot2_kernel<<<g, b>>>(ext, thrust::raw_pointer_cast(&d[0]), max_iter, r);
}
...
void cuda_mandelbrot2()
{
	...
	timer.start();
	mandelbrot2_kernel << <g, b >> >(ext, raw_pointer_cast(&d[0]), max_iter, r);
	cudaDeviceSynchronize();
	timer.stop();
	check_cuda();
